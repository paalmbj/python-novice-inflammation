...
> the same time. The `i`-th element of `zip(my_list, my_other_list)` contains
> the `i`-th element of `my_list` and the `i`-th element of `my_other_list`.
> The length of the sequence generated by `zip` is the same as the shortest 
> of its arguments.
> 
> We are interested to know the distance between each of a list of positions:
> ~~~
> pos = [10, 13, 17, 20, 24, 27, 31]
> ~~~
> {: .python}
> 
> Use `zip` and slicing to write a loop to print the differences between 
> consecutive elements, `pos[1]-pos[0]`
> etc. Hint: the expression `pos[1:]` will give you a list with `pos[1]`
