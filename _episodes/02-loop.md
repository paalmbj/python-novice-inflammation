...
> ## Computing differences with zip
>
> We are interested to know the difference between consecutive values in 
> of a list of position measurements:
>
> ~~~
> pos = [10, 13, 17, 20, 24, 27, 31]
> ~~~
> {: .python}
>
> The built-in function `zip` allows you to loop over multiple lists at 
> the same time. We would like to use it to loop over `pos` and a 
> 
> 
> ~~~
> zip(my_list, my_other_list)
> ~~~
> {: .python}
>
> contains `my_list[i]` and `my_other_list[i]`.
> The length of the sequence generated by `zip` is the same as the shortest 
> of its arguments.
> 
> 
> Use `zip` and slicing to write a loop to print the differences between all
> pairs of consecutive elements, `pos[1]-pos[0]` etc. Hint: the expression
> `pos[1:]` will give you a list with `pos[1]` as its first element.
>
> > ## Solution
> > ~~~
> > for pos0, pos1 in zip(pos, pos[1:]):
> >     print pos1 - pos0
> > ~~~
> > {: .python}
> {: .solution}
